    ...
    match op.flattened::<EntryTypes, LinkTypes>()? {
        FlatOp::StoreEntry(store_entry) => match store_entry {
            OpEntry::CreateEntry { app_entry, action } => match app_entry {
                EntryTypes::CloneServiceRequest(clone_service_request) => {
                    validate_create_clone_service_request(
            ...
            OpUpdate::Entry { app_entry, action } => {
                ...
                match app_entry {
                    EntryTypes::CloneServiceRequest(clone_service_request) => {
                        ...
                        let original_clone_service_request =
                            match CloneServiceRequest::try_from(original_app_entry) {
                                Ok(entry) => entry,
                                Err(e) => {
                                    return Ok(ValidateCallbackResult::Invalid(format!(
                                        "Expected to get CloneServiceRequest from Record: {e:?}"
                                    )));
                                }
                            };
                        validate_update_clone_service_request(
                            action,
                            clone_service_request,
                            original_create_action,
                            original_clone_service_request,
                        )
                    }
                }
            }
        ...
        FlatOp::RegisterDelete(delete_entry) => {
            ...
            match original_app_entry {
                EntryTypes::CloneServiceRequest(original_clone_service_request) => {
                    validate_delete_clone_service_request(
        ...
        FlatOp::RegisterCreateLink {
            ...
            LinkTypes::AllCloneServiceRequests => validate_create_link_all_clone_service_requests(
                action,
                base_address,
                target_address,
                tag,
        ...
        FlatOp::StoreRecord(store_record) => {
            match store_record {
                ...
                OpRecord::UpdateEntry {
                    ...
                    match app_entry {
                        EntryTypes::CloneServiceRequest(clone_service_request) => {
                            let result = validate_create_clone_service_request(
                                EntryCreationAction::Update(action.clone()),
                                clone_service_request.clone(),
                            )?;
                            if let ValidateCallbackResult::Valid = result {
                                let original_clone_service_request: Option<CloneServiceRequest> =
                                    original_record
                                        .entry()
                                        .to_app_option()
                                        .map_err(|e| wasm_error!(e))?;
                                let original_clone_service_request =
                                    match original_clone_service_request {
                                        Some(clone_service_request) => clone_service_request,
                                        None => {
                                            return Ok(
                ...
                OpRecord::CreateLink {
                    ...
                    LinkTypes::AllCloneServiceRequests => {
                        validate_create_link_all_clone_service_requests(
                            action,
                            base_address,
                            target_address,
                            tag,
                ...
                OpRecord::DeleteLink {
                    ...
                    match link_type {
                        LinkTypes::AllCloneServiceRequests => {
                            validate_delete_link_all_clone_service_requests(
                                action,
                                create_link.clone(),
                                base_address,
