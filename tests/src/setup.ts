...
const testHappUrl = dirname(fileURLToPath(import.meta.url)) + '/../../workdir/push-notifications-service-provider_test.happ';
...
async function addPlayer(scenario: Scenario) {
  ...
  const store = new PushNotificationsServiceProviderStore(
  	new PushNotificationsServiceProviderClient(player.appWs as any, 'push_notifications_service_provider_test'),
  );
  return {
  	store,
  	player,
